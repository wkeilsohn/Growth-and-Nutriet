...
G.vec <-c(rep(1, times = length(z)-1), NA)
...
for(x in 1:length(PAR)){
    ...
    graz_max = 1 ####max cells ingested per day at each depth bin
    graz_Km = 100000 ###phytoplantkton consentration where we start to approach graz max
    
    G_mumax = 0.5 #max growth rate for the grazer
    G_Km = 1 ###half saturation constant for grazer growth rate
    GR = 0.9 ### grazer respiration
    GDR = -0.1 ###grazer death rate
    ...
    graz.rate = (graz_max*cell.vec)/(graz_Km+cell.vec)-GR
    
    cell.eaten = graz.rate*G.vec
    
    graz.mu = (G_mumax*graz.rate)/(G_Km+graz.rate)
    
    graz.mu[which(graz.mu<0] <- GDR
    
    G.vec = G.vec*exp(graz.mu)
    
    cell.vec = cell.vec-cell.eaten
    
    cell.vec = replace(cell.vec, cell.vec <0, 1)
